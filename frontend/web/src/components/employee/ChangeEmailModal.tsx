...
const ChangeEmailModal: React.FC<ChangeEmailModalProps> = ({
  ...
  const { user, accessToken, role, cinemaId } = useSelector(selectAuth);
  ...
  const requestMutation = useMutation({
    mutationFn: (email: string) => employeeService.requestUpdateEmail(email),
    onSuccess: () => {
      toast.success(
    ...
    onError: (error: unknown) => {
      const responseData = (
        error as {
          response?: {
            data?: { errors?: { newEmail?: string }; message?: string };
          };
        }
      )?.response?.data;
  ...
  const verifyMutation = useMutation({
    mutationFn: (otpCode: string) =>
      employeeService.verifyUpdateEmail({ newEmail, otp: otpCode }),
    onSuccess: (res: { message?: string; data?: string }) => {
      toast.success(res.message || "Cập nhật email thành công");
      if (user) {
        dispatch(
          setCredentials({
            user: { ...user, email: res.data || newEmail },
    ...
    onError: (error: unknown) => {
      const responseData = (
        error as {
          response?: { data?: { errors?: { otp?: string }; message?: string } };
        }
      )?.response?.data;
  ...
  const otpHeader = (
    <div className="flex flex-col items-center gap-2 mb-8">
      ...
      <p className="text-sm text-slate-500 text-center">
        ...
        <span className="font-semibold text-slate-700">{newEmail}</span>. Vui
  ...
  return (
    <Modal
      isOpen={isOpen}
      onClose={handleClose}
      title={step === "email" ? emailHeader : otpHeader}
    >
      {step === "email" && (
        <form onSubmit={handleEmailSubmit} className="space-y-3">
          ...
          <div className="pt-2">
            <Button
              ...
              className="w-full bg-alpha-blue hover:bg-alpha-blue/90 text-white font-medium rounded-md shadow-lg shadow-orange-100 transition-all active:scale-[0.98] flex items-center justify-center gap-2"
            >
              {requestMutation.isPending ? (
                <>
                  <Loader2 size={18} className="animate-spin" /> ĐANG GỬI OTP...
                </>
              ) : (
                "GỬI MÃ OTP"
              )}
      ...
      {step === "otp" && (
        <form onSubmit={handleOtpSubmit} className="space-y-3">
          <div>
            ...
            {otpError && (
              <p className="text-red-500 text-[10px] mt-2 font-medium text-center">
                {otpError}
              </p>
            )}
          </div>
          ...
          <div className="flex flex-col gap-2 pt-2">
            ...
            >
              {verifyMutation.isPending ? (
                <>
                  <Loader2 size={18} className="animate-spin" /> ĐANG XÁC
                  NHẬN...
                </>
              ) : (
                "HOÀN TẤT"
