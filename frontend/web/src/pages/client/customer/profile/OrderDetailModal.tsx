...
const OrderDetailModal: React.FC<OrderDetailModalProps> = ({
  ...
  const {
    ...
    queryFn: () =>
      orderService.getOrderHistoryDetail(orderId!).then((res) => res.data),
      ...
      <div className="relative w-full max-w-3xl max-h-[calc(100vh-120px)] bg-white rounded-2xl shadow-2xl overflow-hidden animate-in fade-in zoom-in duration-300 flex flex-col mt-18">
        ...
        <div className="flex items-center justify-between px-5 py-4 border-b border-slate-100 bg-white shrink-0">
          ...
          <Button
            onClick={onClose}
            variant="ghost"
            className="h-9 w-9 p-0 rounded-full text-slate-400 hover:text-slate-700 hover:bg-slate-100"
          >
            <X size={20} />
          </Button>
        </div>
            ...
            <div className="p-5 md:p-7 flex flex-col gap-6">
              ...
              <div className="relative bg-white border border-slate-200 rounded-xl overflow-hidden flex flex-col md:flex-row shadow-sm">
                ...
                <div className="flex-1 p-5 md:p-6 relative">
                  ...
                  <div
                    className="absolute top-0 left-0 -translate-y-1/2 w-5 h-5 bg-white border border-slate-200 rounded-full md:hidden"
                    style={{ left: "-10px" }}
                  />
                  <div
                    className="absolute top-0 right-0 -translate-y-1/2 w-5 h-5 bg-white border border-slate-200 rounded-full md:hidden"
                    style={{ right: "-10px" }}
                  />
        ...
        <div className="px-6 py-4 bg-slate-50 border-t border-slate-100 flex justify-center">
          <p className="text-[11px] text-slate-400 text-center">
            Vui lòng xuất mã QR tại quầy vé để nhận vé giấy hoặc vào phòng
            chiếu.
            <br />
