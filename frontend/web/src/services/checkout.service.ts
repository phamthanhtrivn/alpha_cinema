...
export const checkoutService = {
  determineTicketPrice: async (params: {
    ...
    return response.data as { success: boolean; data: DetermineTicketPriceResponse; message?: string };
  },
  createSession: async (request: CreateCheckoutSessionRequest) => {
    const response = await apiClient.post("/checkouts/sessions", request);
    return response.data as { success: boolean; data: CheckoutSessionResponse; message?: string };
  },
  updateSession: async (sessionId: string, request: UpdateCheckoutSessionRequest) => {
    const response = await apiClient.put(`/checkouts/sessions/${sessionId}`, request);
    return response.data as { success: boolean; data: CheckoutSessionResponse; message?: string };
  },
  ...
  confirmSession: async (sessionId: string, request: ConfirmCheckoutSessionRequest, userIp?: string) => {
    const response = await apiClient.post(`/checkouts/sessions/${sessionId}/confirm`, request, {
